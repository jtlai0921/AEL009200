...
namespace Linq_to_DataSet1
{
    public partial class Form1 : Form
    {
        ...
        //建立DataSet物件ds，ds建立於函式之外以便所有事件一起共用
        ...
        private void btnOk_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                var emp = from p in dtEmp.AsEnumerable() where p.Field<int>("薪資") >= int.Parse(txtInput.Text) orderby p.Field<int>("薪資") descending select new { 員工編號 = p.Field<int>("編號"), 員工姓名 = p.Field<string>("姓名"), 員工電話 = p.Field<string>("電話"), 員工職稱 = p.Field<string>("職稱"), 員工薪資 = p.Field<int>("薪資") };
