...
namespace DirectoryInfo_1
{
    class Program
    {
        static void Main(string[] args)
        {  // 建立DirectoryInfo類別的dir物件，可用來操作資料夾目錄
            DirectoryInfo dir = new DirectoryInfo("C:\\CSharp");
            if (dir.Exists)
            {	// 判斷目錄是否存在
                Console.WriteLine("C:\\CSharp路徑存在, 不建立目錄");
            }
            else
            {
                Console.WriteLine("C:\\CSharp路徑不存在，建立目錄");
                dir.Create();	// 建立目錄
                dir.Refresh();	// 重新整理目錄
            }
            ...
            Console.WriteLine();
            Console.Write("是否刪除C:\\CSharp資料夾   1.刪除  2.不刪除->");
